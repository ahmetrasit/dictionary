...
- Select 1-5 target-language glosses and 1-3 excluded or confusable alternatives.
  `semantic_definition` is exactly one sentence and is the broad meaning anchor;
  selected glosses must be
...
- Assess the complete packaged neighbor roster and include every materially useful
  contrast, up to five. Order selected distinctions by reader-facing importance:
  the first row must be the clearest key distinction for a compact dictionary view.
  When only one is useful, return `single_sufficient` with a concrete reason; do not
  stop merely because the schema minimum is one.
